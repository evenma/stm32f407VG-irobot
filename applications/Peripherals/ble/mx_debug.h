...
#define YesOrNo(x) (x ? "YES" : "NO")
    ...
    #define custom_log(N, M, ...) do {rt_kprintf("[%s: %s:%4d] " M "\r\n",  N, SHORT_FILE, __LINE__, ##__VA_ARGS__);}while(0==1)
    #define custom_print(M, ...) do {rt_kprintf( M, ##__VA_ARGS__);}while(0==1)

    #ifndef MX_ASSERT_INFO_DISABLE                                       
        #define debug_print_assert(A,B,C,D,E,F) do {rt_kprintf("[MX:%s:%s:%4d] **ASSERT** %s""\r\n", D, F, E, (C!=NULL) ? C : "" );}while(0==1)
    #else 
        #define debug_print_assert(A,B,C,D,E,F)
    #endif

    #ifdef TRACE
        #define custom_log_trace(N) do {rt_kprintf("[%s: [TRACE] %s] %s()\r\n", N, SHORT_FILE, __PRETTY_FUNCTION__);}while(0==1)
    #else  // !TRACE
